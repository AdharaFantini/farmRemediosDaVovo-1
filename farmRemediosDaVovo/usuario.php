...
<script src="assets/scripts/endereco.js"></script>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($rua)) {
        ...
        $erros["rua"] = "A rua é obrigatória.";

    } elseif (strlen($rua) < 3) {

        $erros["rua"] = "Digite uma rua válida.";
    ...
    } elseif (!preg_match("/^[0-9]+[A-Za-z]?$/", $numero)) {
    ...
    // O complemento pode ficar vazio.
    // Portanto, não é obrigatório.
    ...
    if (empty($bairro)) {
        ...
        $erros["bairro"] = "Digite um bairro válido.";
    ...
    if (empty($cidade)) {
        ...
        $erros["cidade"] = "Digite uma cidade válida.";
    ...
    if (empty($estado)) {
        ...
        $erros["estado"] = "O estado é obrigatório.";

    } elseif (!preg_match("/^[A-Za-z]{2}$/", $estado)) {

        $erros["estado"] = "Digite a sigla do estado. Ex.: SP.";
    ...
    if (empty($erros)) {
        ...
        $estado = htmlspecialchars(strtoupper($estado));


        /*
         * Por enquanto, podemos devolver os dados
         * para a página usando JavaScript/localStorage.
         *
         * Assim você mantém o sistema atual.
         */
